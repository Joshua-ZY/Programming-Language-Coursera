...
class Piece 
  ...
  def initialize (point_array, board)
    ...
    @rotation_index = (0..(@all_rotations.size-1)).to_a.sample  #随机从所有rotation中选一个
    @color = All_Colors.sample
    @base_position = [5, 0] # [column, row] #从[5,0]位置落下来，5应该是canvas的横坐标
    @board = board
    @moved = true
  end
  ...
  # piece用array表示，每一个element都是一个xy坐标，代表一个piece的一个小块，转动只要调整坐标即可
  ...
  All_Pieces = [[[[0, 0], [1, 0], [0, 1], [1, 1]]],  # square (only needs one)
               rotations([[0, 0], [-1, 0], [1, 0], [0, -1]]), # T ,用一个rotation命令生成四种形状的T
